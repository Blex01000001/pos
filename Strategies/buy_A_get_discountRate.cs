...
{
    internal class buy_A_get_discountRate : ADiscount
    {
        public buy_A_get_discountRate(List<Item> itemlist, DiscountStrategy discountStrategy) : base(itemlist)
        {
        ...
        {
            Item item4 = _itemlist.FirstOrDefault(x => x.Name == _itemA & x.Qt >= _itemA_qt);
            if (item4 != null)
            {
                float mPrice = item4.Price * _itemA_qt * (1 - _discountRate);
                Item discount = new Item(_itemA + "折扣$-" + (int)mPrice, item4.Qt / _itemA_qt);
